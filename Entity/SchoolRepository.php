...
class SchoolRepository extends EntityRepository
{
    ...
    public function getWithPrefix($prefix)
    {
        return $this->getEntityManager()
            ->createQuery('SELECT s.id, s.omid, s.name, s.city, s.address, s.postalcode FROM Eotvos\EjtvBundle\Entity\School s WHERE s.name LIKE :prefix AND (s.eductype LIKE :educ1 OR s.eductype LIKE :educ2) AND s.active = true ORDER BY s.name ASC')
